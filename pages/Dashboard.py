...
query9 = """
SELECT c.channel_name, AVG(TIME_TO_SEC(v.duration)) AS average_duration_seconds
FROM channel c
JOIN playlist p ON c.channel_id = p.channel_id
JOIN video v ON p.playlist_id = v.playlist_id
GROUP BY c.channel_name
...
#st.title("Channel Video Count")
plt.figure(figsize=(2, 1))
colors = ['skyblue', 'lightgreen', 'lightcoral', 'lightsalmon', 'lightblue', 'lightseagreen', 'lightskyblue']
plt.bar(result2['channel_name'], result2['video_count'], color=colors)
plt.xlabel("Channel Name", fontsize=5)
plt.ylabel("Video Count", fontsize=5)
plt.title("Video Count by Channel", fontsize=6)
plt.xticks(fontsize=4,rotation=45, ha='right')
desired_y_ticks = [0, 500, 1000, 1500]
plt.yticks(desired_y_ticks,fontsize=4)
